...
<template>
  <div class="convert-page">
    <h2 class="page-title">PDF 合并</h2>
    <p class="page-desc">将多个 PDF 文件按顺序合并为一个文档</p>

    <div v-if="showError" class="error-alert">
      <span>合并失败：{{ errorMessage }}</span>
      <button class="error-close" @click="dismissError">✕</button>
    </div>

    <FileTable />

    <div v-if="fileStore.files.length > 1" class="merge-hint">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        <circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>
      </svg>
      <span>文件将按列表顺序从上到下依次合并。请确保文件按正确顺序排列。</span>
    </div>
